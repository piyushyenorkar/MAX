...
def process_command(user_input: str):
    ...
    if any(word in user_input.lower() for word in ["exit", "quit", "goodbye", "bye max", "stop"]):
...
def run_voice_agent():
    ...
    Works like a real conversation: MAX speaks, you speak, repeat.
    """
    state.running = True
    state.mode = "listening"
    log("info", "MAX Voice Agent started")
    ...
    state.running = False
    state.mode = "idle"
    log("info", "MAX Voice Agent stopped")
...
def run_voice_loop():
    ...
    state.running = True
    state.mode = "idle"
    log("info", "MAX started in wake-word voice mode")
    ...
    state.running = False
    state.mode = "idle"
    log("info", "MAX stopped")


def run_text_loop():
    """Text-based command loop (fallback)."""
    state.running = True
    log("info", "MAX started in text mode")
    greeting = _get_time_greeting()
    speak(greeting)
