"""
tools/computer.py
All computer control functions - the hands of Lappy
...
def set_reminder(message: str, seconds: int = 60) -> str:
    ...
    def _remind():
        ...
        try:
            if OS == "Windows":
                run_command(f'msg * "LAPPY REMINDER: {message}"')
            elif OS == "Darwin":
                run_command(f'osascript -e \'display notification "{message}" with title "Lappy Reminder"\'')
        except:
            pass
