"""
ui/dashboard.py
Beautiful Tkinter dashboard for Lappy
...
def launch_gui():
    ...
    from lappy import state, process_command, run_voice_loop, check_api_key
    ...
    tk.Label(header, text="⚡ LAPPY", font=("Courier New", 20, "bold"),
    ...
    chat_box.tag_configure("lappy", foreground=ACCENT, font=("Courier New", 10, "bold"))
    ...
    def chat_append(role: str, text: str, tool: str = None):
        ...
        if role == "user":
            ...
            chat_box.insert("end", f"\n[{ts}] ", "time")
            chat_box.insert("end", "LAPPY › ", "lappy")
    ...
    def send_command(event=None):
        ...
        def run():
            ...
            root.after(0, lambda: chat_append("lappy", resp, tool))
    ...
    def update_status(s):
        colors = {
            ...
            "speaking": (ACCENT, "SPEAKING", "Lappy is responding..."),
        }
        mode = s.mode
    ...
    from lappy import _get_time_greeting
    greeting = _get_time_greeting()
    chat_append("lappy", greeting)
    chat_append("lappy", f"🎤 Voice mode is active — just say '{WAKE_WORD}' and talk to me!")
